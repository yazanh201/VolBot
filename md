...
async def run(config_path: str = "config.yaml", cache=None):
    ...
    async def monitor_tp_sl():
        ...
        - SL מתעדכן תמיד:
            • בפעם הראשונה → entry price
            • לאחר מכן → מחיר סגירה של נר (close candle) ± tolerance, ורק בכיוון שמקטין סיכון.
        ...
        while True:
            ...
            for sym_key, trade_obj in list(open_trades.items()):
                try:
                    ...
                    updates_sl   = trade_obj.get("updates_sl", 0)  # 👈 חדש: מונה עדכוני SL
                    ...
                    if sl_tol > 0:
                        if updates_sl == 0:
                            # 🟢 פעם ראשונה בלבד → קובע SL ל-entry
                            ...
                            if resp.get("success") or str(resp.get("code")) in ("0", "200"):
                                ...
                                trade_obj["updates_sl"] = 1  # 👈 מעלים את המונה
                                logging.info(f"✅ [{sym_key}] SL הוגדר בפעם הראשונה ל-entry: {new_sl}")
                            else:
                                logging.warning(f"⚠️ [{sym_key}] עדכון SL ראשון נכשל → {resp}")
                            continue  # ממשיכים לסיבוב הבא בלולאה

                        elif closed_price:
                            # מהפעם השנייה והלאה → לפי סגירת נרות ± tol
                        ...
                        if new_sl:
                            ...
                            if (side == 1 and sl_price and new_sl < sl_price) or (side == 3 and sl_price and new_sl > sl_price):
                                ...
                                if resp.get("success") or str(resp.get("code")) in ("0", "200"):
                                    trade_obj["sl_price"] = new_sl
                                    trade_obj["updates_sl"] = updates_sl + 1  # 👈 סופרים עדכון נוסף
